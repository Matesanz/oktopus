...
from scripts.const import PATH_DATA, QDRANT_COLL_NAME, QDRANT_LOCAL_PATH, \
    MODEL_EMB_DIM, MODEL_NAME
...
def _init_vectors_staff() -> tuple[SentenceTransformer, QdrantClient]:
    ...
    client.recreate_collection(
        ...
        vectors_config=VectorParams(size=MODEL_EMB_DIM, distance=Distance.COSINE),
    )
    model = SentenceTransformer(MODEL_NAME)
    assert model.get_sentence_embedding_dimension() == MODEL_EMB_DIM, f"{model.get_sentence_embedding_dimension() = } VS {MODEL_EMB_DIM = }"
    return model, client
...
def _populate_db_qdrant():
    ...
    num_docs = len(list(PATH_DATA.glob('*.md')))
    points = [(0.0, 0.0)]
    for doc_idx, md_doc in enumerate(PATH_DATA.glob("*.md")):
